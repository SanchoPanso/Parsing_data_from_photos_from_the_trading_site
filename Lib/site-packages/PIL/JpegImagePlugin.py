...
# 1997-08-27 fl   Save mode 1 images_for_experiments as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        raise SyntaxError(f"cannot handle {self.layers}-layer images_for_experiments")
...
##
# Image plugin for JPEG and JFIF images_for_experiments.
...
def _getmp(self):
    ...
    # it's an error not to have a number of images_for_experiments
    try:
        quant = mp[0xB001]
    except KeyError as e:
        raise SyntaxError("malformed MP Index (no number of images_for_experiments)") from e
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    # There's no subsampling when images_for_experiments have only 1 layer
    # (grayscale images_for_experiments) or when they are CMYK (4 layers),
